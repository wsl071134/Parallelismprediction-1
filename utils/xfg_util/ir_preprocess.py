...
def basic_block_leaf(G, node, ids_in_basic_block):
    """
    Test whether the node is a leaf node of a basic block 测试节点是否是基本块的叶节点
    :param G: Graph
...
def add_stmts_to_graph(G, file, functions_defined_in_file, functions_declared_in_file):
    ...
    for i, line in enumerate(file):
        ...
        ################################################################################################################
        # Add nodes and edges according to statement characteristics根据语句特征添加节点和边
        ...
        ################################################################################################################
        # Variable assignment (except function calls)变量赋值（函数调用除外）
    ...
    # Put things together 讲可以连接的外部调用代码嵌入
    for k, v in functions_defined_in_file.items():
        if v[1]:
...
def construct_xfg_single_raw_folder(params, num_folder):
    ...
    if os.path.exists(data_preprocessing_broken_filename):
        ...
        if not os.path.exists(data_read_from_folder_filename):
            ...
            rgx.safe_pickle([raw_data, file_names], data_read_from_folder_filename)
        ...
        if not os.path.exists(data_preprocessed_filename):
            ...
            rgx.safe_pickle([preprocessed_data, functions_declared_in_files,
        ...
        for i, (preprocessed_file, file_name) in enumerate(zip(preprocessed_data, file_names)):
            ...
            if not os.path.exists(dual_graphs_filename):
                ...
                # Save G to file
                rgx.safe_pickle(G, os.path.join(graph_folder, file_name[:-3] + '_xfg.p'))
                ...
                rgx.safe_pickle(stmt_list, write_to)
                ...
                if not os.path.exists(dual_graphs_filename):
                    ...
                    rgx.safe_pickle(D, dual_graphs_filename)
            else:
